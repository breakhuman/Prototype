...
from conversationtg import start, help
...
def b64decode(update, context):
    if len(update.message.text.split()) == 1:
        update.message.reply_text("/b64d `코드` 이런식으로 입력해주세요.", parse_mode=ParseMode.MARKDOWN)
        return
...
def main():
    updater = Updater(id, use_context=True)  # Place your token in config.py
